...
from utils.beam_omt_trs import Translator as TrsTranslator
import pprint
...
class Attentive(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.attn = nn.Parameter(torch.randn(1, 1, self.hidden_size), requires_grad=True)
        
    def forward(self, x):
        attn_score = F.sigmoid(torch.sum(x * self.attn, dim=2)).unsqueeze(2)
        #print(attn_score.shape)
        #print(x.shape)
        #print((x * self.attn).shape)
        return x * attn_score
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        self.layer_norm_ffn = LayerNorm(hidden_size)
        # self.layer_norm_end = LayerNorm(hidden_size)
        self.attentive = Attentive(hidden_size)
        ...
        for line in open(config.emb_file).readlines():
            ...
            if(len(sp) == config.emb_dim + 1):
                if sp[0] in vocab.word2index:
                    pre_trained += 1
                    embeddings[vocab.word2index[sp[0]]] = [float(x[7:-1]) for x in sp[1:]]
            else:
                print(sp[0])
...
def log_all(dial,ref,hyp_b,per,log_file):
    ...
    for i in range(len(ref)):
        ...
        f.write("generated: {}".format(hyp_b[i]))
        f.write("\n")
...
def evaluate_vae(model, data, model_name='trs', ty='valid', writer=None, n_iter=0, ty_eval="before", verbose=False, log=False, result_file="results2/results_vae.txt", ref_file="results2/ref_vae.txt", case_file="results2/case_vae.txt"):
    if log:
    ...
    for j, batch in pbar:
        ...
        loss, ppl, _, _, _, _ = model.train_one_batch(batch, train=False)
        l.append(loss)
        p.append(ppl)
...
def evaluate_transformer(model, data, model_name='trs', ty='valid', writer=None, n_iter=0, ty_eval="before", verbose=False, log=False, result_file="results/results_transformer.txt", ref_file="results/ref_transformer.txt", case_file="results/case_transformer.txt"):
    if log:
        f1 = open(result_file, "a")
        f2 = open(ref_file, "a")
    dial,ref, hyp_b, per= [],[],[], []
