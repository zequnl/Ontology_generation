...
def prepare_data_seq():
    ...
    for l in test_name:
        node_dic[l] = len(node_dic)
    for k in graph:
        #if k not in node_dic
        node1 = node_dic[k]
        node2 = node_dic[graph[k]]
    ...
    with open(config.save_path+'dataset.p', "wb") as f:
        ...
        print("Saved PICKLE")
    with open(config.save_path+'/graph.txt','w') as f:
        for k in adj_lists:
            f.write(str(k))
            f.write(" ")
            for n in adj_lists[k]:
                f.write(str(n))
                f.write(",")
            f.write("\n")
    ...
    return train, valid, test, vocab, adj_lists, node_dic

class Data:
    def __init__(self):
        self.train, self.valid, self.test, self.vocab, self.graph, self.node_dic = prepare_data_seq()
        self.type = {'train': self.train, 'valid': self.valid, 'test': self.test} 
        '''
        if(os.path.exists(config.save_path_dataset+'graph.json')):
            with open(config.save_path_dataset+'dataset.p', "rb") as f:
                [self.train, self.valid, self.test, self.vocab] = pickle.load(f)
            with open(config.save_path_dataset+'graph.json', "r") as f:
                self.graph = json.load(f)
            with open(config.save_path_dataset+'node_dic.json', "r") as f:
                self.node_dic = json.load(f)
            self.type = {'train': self.train,
                        'valid': self.valid,
                        'test': self.test}
